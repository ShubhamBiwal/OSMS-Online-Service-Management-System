...
function sendMail($email, $code)
{
    ...
    try {
        ...
        $mail->Username   = 'shubhambiwa@gmail.com';                     //SMTP username
        $mail->Password   = 'layllemfzdliahlg';                               //SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
        $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

        //Recipients
        $mail->setFrom('shubhambiwa@gmail.com', 'OSMS');
...
if (isset($_POST['resetbtn'])) {
    ...
    $sql = "SELECT tech_email FROM technician_tb WHERE tech_email = '$foremail'";
    $run = mysqli_query($conn, $sql);

    if ($result = mysqli_fetch_array($run)) {
        if ($foremail == $result['tech_email']) {
            $_SESSION['forgot-email'] = $result['tech_email'];
            $code = rand(100000, 999999);

            $sql3 = "UPDATE technician_tb SET code = '$code' WHERE tech_email = '$foremail' ";
...
if (isset($_POST['otpbtn'])) {
    $otp = $_POST['otp'];
    $temail = $_SESSION['forgot-email'];
    $sql = "SELECT code FROM technician_tb WHERE tech_email = '$temail'";
    $run = mysqli_query($conn, $sql);
...
if (isset($_POST['submit-btn'])) {
    ...
    if ($newpass == $confirmpass) {
        ...
        $sql2 = "UPDATE technician_tb SET tech_password = '$newpass' WHERE tech_email = '$forgotemail' ";
        $run2 = mysqli_query($conn, $sql2);

        if ($run2) {
            echo '<script>alert("Password Updated Successfully.")</script>';
            echo '<script>window.location="/osms/technician";</script>';
        }
    } else {
...
<body>
    ...
    <div class="container register">
        <p><a href="/osms/technician">Go Back</a></p>
    </div>
