...
if (isset($_POST['resetbtn'])) {
    ...
    $sql = "SELECT a_email FROM admin_login WHERE a_email = '$foremail'";
    $run = mysqli_query($conn, $sql);

    if ($result = mysqli_fetch_array($run)) {
        if ($foremail == $result['a_email']) {
            $_SESSION['forgot-email'] = $result['a_email'];
            $code = rand(100000, 999999);

            $sql3 = "UPDATE admin_login SET a_otp = '$code' WHERE a_email = '$foremail' ";
...
if (isset($_POST['otpbtn'])) {
    $otp = $_POST['otp'];
    $aemail = $_SESSION['forgot-email'];
    $sql = "SELECT a_otp FROM admin_login WHERE a_email = '$aemail'";
    $run = mysqli_query($conn, $sql);
...
if (isset($_POST['submit-btn'])) {
    ...
    if ($newpass == $confirmpass) {
        ...
        $sql2 = "UPDATE admin_login SET a_password = '$newpass' WHERE a_email = '$forgotemail' ";
        $run2 = mysqli_query($conn, $sql2);

        if ($run2) {
            echo '<script>alert("Password Updated Successfully.")</script>';
            echo '<script>window.location="/osms/admin";</script>';
        }
    } else {
...
<body>
    ...
    <div class="container register">
        <p><a href="/osms/admin">Go Back</a></p>
    </div>
